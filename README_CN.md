...
T800 交付版的跳舞任务使用仅训练 Punch 的策略包，只暴露 `Punch_Swing_L_50hz.npz` 一个动作。其他跳舞轨迹和动作切换快捷入口不会包含在本交付版中。Punch 动作结束后按照 `assets/config/t800/task_motion/default.yaml` 中的状态机配置进行后续切换。
