...
T800 交付版的跳舞任务使用 model 83500 的 obs-expanded tracking 策略，包含 `Punch_Swing_L_50hz.npz`、`Punch_Swing_L_fk_50hz.npz`、`kick_Turn_50hz.npz`、`riot_combo_50hz.npz` 和 `victory_50hz.npz`。需要先从 `pd_stand` 进入 dance（`LB + A`，再 `RB + B`）；随后可以通过虚拟手柄的 dance 控件切换 Punch、Punch FK、Kick Turn、Riot Combo 或 Victory。软急停仍然是 `LB + RB`。
