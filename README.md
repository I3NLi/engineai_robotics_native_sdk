...
The T800 delivery dance task uses the model 83500 observation-expanded tracking policy and includes `Punch_Swing_L_50hz.npz`, `Punch_Swing_L_fk_50hz.npz`, `kick_Turn_50hz.npz`, `riot_combo_50hz.npz`, and `victory_50hz.npz`. Enter dance from `pd_stand` (`LB + A`, then `RB + B`); choose Punch, Punch FK, Kick Turn, Riot Combo, or Victory from the virtual gamepad dance controls. Soft emergency fallback remains `LB + RB`.
